...
import com.cmdt.carday.microservice.model.request.alert.AlertCreateDto;
...
import com.cmdt.carday.microservice.model.request.alert.AlertOrgStaticDto;
import com.cmdt.carday.microservice.model.response.alert.AlertDailyStatisticRetDto;
import com.cmdt.carday.microservice.model.response.alert.AlertOrgRetDto;
import com.cmdt.carday.microservice.model.response.alert.AlertOrgStatisticRetDto;
...
//@Service
public class PlatformAlertStatisticService
{
    
    private static final Logger LOG = LoggerFactory.getLogger(PlatformAlertStatisticService.class);
    ...
    public AlertOrgStatisticRetDto statisticAlertByTypeAndTimeRanger(AlertOrgStaticDto dto)
    {
        ...
        List<Organization> orgList = organizationService.findDownOrganizationListByOrgId(dto.getEntId(), false, true);
        if (orgList != null && orgList.size() > 0)
        {
            List<AlertStatisticModel> data = vehicleAlertJobDao.statisticAlertByTypeAndTimeRanger(orgList,
                dto.getEntId(),
                false,
                true,
                dto.getType(),
        ...
        {
            return new AlertOrgStatisticRetDto(dto.getType(), dto.getFrom(), dto.getTo(),
                new ArrayList<AlertStatisticModel>());
        }
    }
    ...
    public AlertDailyStatisticRetDto statisticDailyAlertByTypeAndTimeRanger(AlertOrgDailyStatisticDto dto)
    {
        ...
        List<Organization> orgList =
            organizationService.findDownOrganizationListByOrgId(Long.parseLong(dto.getEntId()), false, true);
        if (orgList != null && orgList.size() > 0)
        {
            ...
            List<CountModel> result = vehicleAlertJobDao.statisticDailyAlertByTypeAndTimeRanger(orgIdList,
                Long.parseLong(dto.getEntId()),
                false,
                dto.getType(),
                dto.getFrom(),
                dto.getTo());
            ...
            {
                data.add(new DateCountModel(model.getCountDate(),
                    model.getCountValue() == null ? 0 : model.getCountValue()));
            }
            return new AlertDailyStatisticRetDto(dto.getType(), dto.getFrom(), dto.getTo(), data);
        }
        else
        {
            return new AlertDailyStatisticRetDto(dto.getType(), dto.getFrom(), dto.getTo(),
                new ArrayList<DateCountModel>());
        }
    }
    
    public AlertOrgRetDto queryAlertByTypeAndTimeRanger(AlertOrgDto dto)
    {
        PagModel page = null;// vehicleAlertJobDao.queryAlertByOrgTypeAndTimeRange(dto.getType(),dto.getFrom(),dto.getTo(),dto.getDepartmentId(),dto.getCurrentPage(),dto.getPageSize());
        ...
        return new AlertOrgRetDto(dto.getType(), dto.getFrom(), dto.getTo(), dto.getPageSize(), dto.getCurrentPage(),
            page.getTotalRows(), alertList);
    }
    ...
    public void generateAlertByType(AlertCreateDto alertCreateDto)
    {
        ...
        alert.setAlertTime(new Timestamp(
            DateUtils.string2Date(alertCreateDto.getTraceTime(), DateUtils.FORMAT_YYYY_MM_DD_HH_MI_SS).getTime()));
        ...
        if (null != vehicleModel)
        {
            ...
            {
                double os =
                    (double)(alertCreateDto.getSpeed() - vehicleModel.getLimitSpeed()) / vehicleModel.getLimitSpeed();
            ...
            DriverModel driverModel = vehicleAlertJobDao.findDriver(vehicleModel.getVehicleNumber(),
                DateUtils.string2Date(alertCreateDto.getTraceTime(), DateUtils.FORMAT_YYYY_MM_DD_HH_MI_SS));
            if (null != driverModel)
            {
    ...
    private void sendAlertPush(List<VehicleAlertModel> alertData, boolean isRelease)
    {
        ...
        for (VehicleAlertModel alert : alertData)
        {
            ...
            {
                List<UserModel> admins = userDao.listOrgAdminListByOrgId(alert.getCurrentUseOrgId());
                if (admins != null && admins.size() > 0)
                {
                    ...
                    if (mobiles.size() > 0)
                    {
                        if (isRelease)
                        {
                            ...
                            String res = commService.sendPush(mobiles,
                                Constants.ALERT_NOTIFICATION_TITLES.get(alert.getAlertType()),
                                MessageTemplateUtil.composeReleaseOutBoundAlertMsg(alert),
                                Constants.CARDAY_ADMIN);
                        ...
                        else
                        {
                            String res = commService.sendPush(mobiles,
                                Constants.ALERT_NOTIFICATION_TITLES.get(alert.getAlertType()),
                                MessageTemplateUtil.composeMessageForAlert(alert),
                                Constants.CARDAY_ADMIN);
    ...
    private void saveAlertToMessage(List<VehicleAlertModel> alertData, boolean isRealse)
    {
        ...
        for (VehicleAlertModel alert : alertData)
        {
            ...
            msg.setType(alert.getAlertType().equals(AlertType.OUTBOUND.toString()) ? Message.MessageType.OUTBOUND
                : (alert.getAlertType().equals(AlertType.OVERSPEED.toString()) ? Message.MessageType.OVERSPEED
                    : Message.MessageType.VEHICLEBACK));
    ...
    private Point transGpsPoint(double lon, double lat)
    {
        ...
        try
        {
            Map<String, Object> result =
                new ServiceAdapter(shouqiService).doService(ActionName.TRANSGPSTOBAIDU, new Object[] {list});
