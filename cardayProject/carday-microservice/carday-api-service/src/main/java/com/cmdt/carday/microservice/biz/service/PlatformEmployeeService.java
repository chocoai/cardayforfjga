...
public class PlatformEmployeeService {
	...
	public User createEmployee(User loginUser,EmployeeCreateDto dto){
		...
		if(loginUser.isEntAdmin() || loginUser.isDeptAdmin()){
			...
			return userService.createEmployee(user, emp);
		 } else {
			 throw new ServiceException(MessageCode.COMMON_NO_AUTHORIZED);
		 }
	}
	...
	public boolean updateEmployee(User loginUser,EmployeeUpdateDto dto){
		...
		if(loginUser.isEntAdmin() || loginUser.isDeptAdmin()){
    		...
    		if(emp != null){
    			...
    			throw new ServiceException(MessageCode.EMP_NOT_EXIST);
    		}
		}else{
			throw new ServiceException(MessageCode.COMMON_NO_AUTHORIZED);
		}
	...
	public String importEmployeeFromFile(User loginUser,MultipartFile multiFile) throws Exception{
