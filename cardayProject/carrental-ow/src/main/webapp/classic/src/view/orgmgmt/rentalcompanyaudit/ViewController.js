 ...
 */
Ext.define('Admin.view.orgmgmt.rentalcompanyaudit.ViewController', {
	extend : 'Ext.app.ViewController',
	requires : [
			//'Admin.view.orgmgmt.rentalcompanyaudit.refuseWindow'
			],
	onBeforeload: function(){
		console.log('onBeforeLoad');
		var frmValues = this.lookupReference('searchForm').getValues();
		var page=Ext.getCmp('auditRentalIdPage').store.currentPage;
		var limit=Ext.getCmp('auditRentalIdPage').pageSize;
	...
	onSearchClick:function(){
		...
		var EnterInfoStore = this.lookupReference('gridAuditRentalInfo').getStore();
		EnterInfoStore.currentPage = 1;
		this.getViewModel().getStore('auditResults').load();
	},

	checkRentalAuditInfo: function(grid, rowIndex, colIndex){
		var rec = grid.getStore().getAt(rowIndex);
		var win = Ext.widget('checkRentalAuditInfo');
		win.down("form").loadRecord(rec);
		win.show();

		if(rec.data.businessType != ''){
			if(rec.data.businessType.indexOf('自有车') >= 0){
				Ext.getCmp("businessTypeRentalAudit").items.items[0].setValue(true);
			}
			if(rec.data.businessType.indexOf('长租车') >= 0){
				Ext.getCmp("businessTypeRentalAudit").items.items[1].setValue(true);
			}
		}
	},
...
//	        headers : {'Content-type' : 'application/json;utf-8'},
	        success : function(response,options) {
				...
				var	retStatus = respText.status;
					if (retStatus == 'success') {
						btn.up('openRentalWindow').close();
						Ext.getCmp('auditRentalId').getStore("auditResults").load();
					}else{
						btn.up('openRentalWindow').close();
	...
	onOpenClick:function(grid, rowIndex, colIndex){
		var win = Ext.widget("openRentalWindow");
		var rec = grid.getStore().getAt(rowIndex);	
	...
	afterrenderOpenRentalWindow: function(){
...
//	        headers : {'Content-type' : 'application/json;utf-8'},
	        success : function(response,options) {
				...
				var	retStatus = respText.status;
					if (retStatus == 'success') {
						btn.up('refuseRentalWindow').close();
						Ext.getCmp('auditRentalId').getStore("auditResults").load();
					}else{
						btn.up('refuseRentalWindow').close();
	...
	onRefuseClick:function(grid, rowIndex, colIndex){
		var win = Ext.widget("refuseRentalWindow");
		var rec = grid.getStore().getAt(rowIndex);	
		win.down("form").getForm().setValues({'id':rec.get('id')});
		win.show();
	},
...
//	        headers : {'Content-type' : 'application/json;utf-8'},
	        success : function(response,options) {
				...
				var	retStatus = respText.status;
					if (retStatus == 'success') {
						btn.up('extendRentalWindow').close();
						Ext.getCmp('auditRentalId').getStore("auditResults").load();
					}else{
						btn.up('extendRentalWindow').close();
						Ext.Msg.alert('消息提示','续约服务失败！');
	...
	onExtendClick:function(grid, rowIndex, colIndex){
		var win = Ext.widget("extendRentalWindow");
		var rec = grid.getStore().getAt(rowIndex);	
		var serviceDate = Ext.util.Format.date(rec.data.startTime, "Y/m/d")+'一'+ Ext.util.Format.date(rec.data.endTime, "Y/m/d");
		win.down("form").getForm().setValues({'id':rec.get('id')});
		win.down("form").getForm().setValues({'entName':rec.get('name')});
