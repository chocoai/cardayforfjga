Ext.define('Admin.view.systemconfiguration.vehSendConfiguration.ViewController', {
	...
	onBeforeLoad : function() {
        ...
        var page=Ext.getCmp('vehSendPage').store.currentPage;
        var limit=Ext.getCmp('vehSendPage').pageSize;
        var input = {
            "currentPage" : page,
            "numPerPage" : limit,
        };
        var pram = Ext.encode(input);
        this.getViewModel().getStore("vehSendResults").proxy.extraParams = {
            "json" : pram
        }
    },

    onSearchClick :　function() {
        var VehicleStore = Ext.getCmp('gridVehSendConfiguration').getStore();
        VehicleStore.currentPage = 1;
        this.getViewModel().getStore("vehSendResults").load();
    },

    viewVehSend : function(grid, rowIndex, colIndex) {
        var rec = grid.getStore().getAt(rowIndex);
		var win = Ext.create('Admin.view.systemconfiguration.vehSendConfiguration.ViewVehSend');
    ...
    addVehSend : function() {
        var win = Ext.create('Admin.view.systemconfiguration.vehSendConfiguration.AddVehSend');
        win.show();
    },

    updateVehSend : function(grid, rowIndex, colIndex){
        var rec = grid.getStore().getAt(rowIndex);
        var win = Ext.create('Admin.view.systemconfiguration.vehSendConfiguration.UpdateVehSend');
        win.down("form").loadRecord(rec);
        switch(rec.data.approvalRating){
            case '0':
    ...
    deleteVehSend : function(grid, rowIndex, colIndex) {
        Ext.Msg.confirm('消息提示','确定要删除此派车审批设置吗?',function(btn){
            if (btn == 'yes') {
                console.log('Delete Vehicle Send Configuration Information!');
    ...
    openAdminChooseWin: function(combo, event, eOpts){
        var win = Ext.create("Admin.view.systemconfiguration.vehSendConfiguration.AdminChooseWin",{
            adminId:combo.up().id
        });
    ...
    chooseAdmin: function(btn, e, eOpts){
        ...
        if(Ext.getCmp("addVehSendWindow")){            
            var form = Ext.getCmp("addVehSendWindow").down('form').getForm();
        }else{
            var form = Ext.getCmp("updateVehSendWindow").down('form').getForm();            
        }
