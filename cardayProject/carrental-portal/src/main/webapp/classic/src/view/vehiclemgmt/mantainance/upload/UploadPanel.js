 ...
 * @since 2012-11-15
 */
Ext.define('Admin.view.vehiclemgmt.mantainance.upload.UploadPanel',{
	...
	columns : [
        {xtype: 'rownumberer'},
		{header: '文件名', width: 100,dataIndex: 'name',sortable: false,menuDisabled: true,align: 'center',},
		{header: '自定义文件名', width: 130,dataIndex: 'fileName',sortable: false,menuDisabled: true,align: 'center',editor: {xtype: 'textfield'}},
        {header: '类型',sortable: false,menuDisabled: true,align: 'center',width: 70,dataIndex: 'type'},
        {header: '大小', sortable: false,menuDisabled: true,align: 'center',width: 70,dataIndex: 'size',renderer:function(v){
        	return Ext.util.Format.fileSize(v);
        }},
        {header: '进度', sortable: false,menuDisabled: true,align: 'center',width: 130,dataIndex: 'percent',renderer:function(v){        	
			var stml =
				'<div>'+
        ...
        {header: '状态',sortable: false,menuDisabled: true,align: 'center', width: 80,dataIndex: 'status',renderer:function(v){
			var status;
			if(v==-1){
				status = "等待上传";
			}else if(v==-2){
