 ...
 */
Ext.define('Admin.view.reportmgmt.vehiclestatistics.ViewController', {
	extend : 'Ext.app.ViewController',
	requires : [ 
			'Ext.window.MessageBox'
	],
	alias : 'controller.vehiclestatisticscontroller',
	...
	onBeforeLoad : function() {		
 		...
 		this.getViewModel().getStore("vehicleList").load();
			...
			if(Ext.getCmp('vehiclestatistics_SearchForm_id').getForm().isValid()){
				var page=Ext.getCmp('GridVehicle_page_vehiclestatistics').store.currentPage;
				var limit=Ext.getCmp('GridVehicle_page_vehiclestatistics').store.pageSize;

				parm = {
				    "currentPage" 	: page,
					"numPerPage" : limit,
					"deptName": searchParam.deptName,
	...
	onExcelClick :　function() {
		...
		if(Ext.getCmp('vehiclestatistics_SearchForm_id').getForm().isValid()){	
			...
			window.location.href = './resources/template/reportmgmt/vehicle_report.xlsx';
		}else{
        	Ext.Msg.alert('消息提示','请输入有效的筛选条件！');
        }
	},
	...
	openDeptChooseWin: function(combo, event, eOpts){
     	var win = Ext.create("Admin.view.reportmgmt.vehiclestatistics.DeptChooseWin",{
  	...
  	chooseDept: function(btn, e, eOpts){
     	...
     	var form = Ext.getCmp("vehiclestatistics_SearchForm_id").getForm();
