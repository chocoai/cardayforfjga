...
Ext.define('Admin.view.alertMgmt.violateAlarm.ViewController', {
	extend : 'Ext.app.ViewController',
	requires : [
	        'Ext.window.MessageBox',
			'Admin.view.alertMgmt.violateAlarm.View',
			'Admin.view.alertMgmt.violateAlarm.SearchForm'
			],
	alias : 'controller.violateAlarmController',
 	...
 	onBeforeLoad:function(){
    	...
    	if(this.lookupReference('searchForm').getForm().isValid()){
	    	var page=Ext.getCmp('violatePage').store.currentPage;
			var limit=Ext.getCmp('violatePage').pageSize;
			frmValues.currentPage=page;
			frmValues.numPerPage=limit;
			this.getViewModel().getStore("violateResults").proxy.extraParams = {
	...
	onSearchClick:function(){
		var VehicleStore = this.lookupReference('violateVehicleGrid').getStore();
		VehicleStore.currentPage = 1;
		this.getViewModel().getStore("violateResults").load();
	},
	
	//查看marker和轨迹
	viewViolateAlarm:function(grid, rowIndex, colIndex){
		var rec = grid.getStore().getAt(rowIndex);	
		var win = Ext.widget("violateAndTrackWindow");
		win.show();
	...
	viewGeofenceMapinfo: function(rec,win){
        var map = Ext.getCmp('abnormal_violateAlarm').bmap;
		this.initMap(map);
		var input={
			...
			success: function(res){
				...
				if(appendData.status=='success'){
					...
					//		alert('从大渡口区到江北区的距离是：'+(map.getDistance(pointA,pointB)).toFixed(2)+' 米。');  //获取两点距离,保留小数点后两位
						...
						win.getViewModel().getStore("violateResults").loadData(tData);
...
onExcelClick:function(){
	...
	if(this.lookupReference('searchForm').getForm().isValid()){
		window.location.href = 'vehicleAlert/exportViolateAlarmAlert?json='  + Ext.encode(frmValues);
    }else{
    	Ext.Msg.alert('消息提示','请输入有效的筛选条件！');
    }
	...
	openDeptChooseWin: function(combo, event, eOpts){
     	var win = Ext.create("Admin.view.alertMgmt.violateAlarm.DeptChooseWin",{
    ...
    chooseDept: function(btn, e, eOpts){
     	...
     	var form = Ext.getCmp("violateAlarmSearchForm").getForm();
