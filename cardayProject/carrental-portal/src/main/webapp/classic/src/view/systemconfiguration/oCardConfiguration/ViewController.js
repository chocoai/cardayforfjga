Ext.define('Admin.view.systemconfiguration.oCardConfiguration.ViewController', {
	...
	onBeforeLoad : function() {
        ...
        var page=Ext.getCmp('oCardPage').store.currentPage;
        var limit=Ext.getCmp('oCardPage').pageSize;
        var input = {
            "currentPage" : page,
            "numPerPage" : limit,
        };
        var pram = Ext.encode(input);
        this.getViewModel().getStore("oCardResults").proxy.extraParams = {
            "json" : pram
        }
    },

    onSearchClick :　function() {
        var VehicleStore = Ext.getCmp('gridOCardConfiguration').getStore();
        VehicleStore.currentPage = 1;
        this.getViewModel().getStore("oCardResults").load();
    },

    viewOCard : function(grid, rowIndex, colIndex) {
        var rec = grid.getStore().getAt(rowIndex);
		var win = Ext.create('Admin.view.systemconfiguration.oCardConfiguration.ViewOCard');
    ...
    addOCard : function() {
        var win = Ext.create('Admin.view.systemconfiguration.oCardConfiguration.AddOCard');
        win.show();
    },

    updateOCard : function(grid, rowIndex, colIndex){
        var rec = grid.getStore().getAt(rowIndex);
        var win = Ext.create('Admin.view.systemconfiguration.oCardConfiguration.UpdateOCard');
        win.down("form").loadRecord(rec);
        switch(rec.data.approvalRating){
            case '0':
    ...
    deleteOCard : function(grid, rowIndex, colIndex) {
        Ext.Msg.confirm('消息提示','确定要删除此O牌审批设置吗?',function(btn){
            if (btn == 'yes') {
                console.log('Delete O Card Configuration Information!');
    ...
    openAdminChooseWin: function(combo, event, eOpts){
        var win = Ext.create("Admin.view.systemconfiguration.oCardConfiguration.AdminChooseWin",{
            adminId:combo.up().id
        });
    ...
    chooseAdmin: function(btn, e, eOpts){
        ...
        if(Ext.getCmp("addOCardWindow")){            
            var form = Ext.getCmp("addOCardWindow").down('form').getForm();
        }else{
            var form = Ext.getCmp("updateOCardWindow").down('form').getForm();            
        }
