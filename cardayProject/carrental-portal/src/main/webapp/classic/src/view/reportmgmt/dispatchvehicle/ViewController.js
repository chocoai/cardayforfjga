 ...
 */
Ext.define('Admin.view.reportmgmt.dispatchvehicle.ViewController', {
	extend : 'Ext.app.ViewController',
	requires : [ 
			'Ext.window.MessageBox'
	],
	alias : 'controller.dispatchvehiclecontroller',
	...
	onBeforeLoad : function() {		
 		...
 		this.getViewModel().getStore("vehicleList").load();
			...
			if(Ext.getCmp('dispatchvehicle_SearchForm_id').getForm().isValid()){
				var page=Ext.getCmp('GridVehicle_page_dispatchvehicle').store.currentPage;
				var limit=Ext.getCmp('GridVehicle_page_dispatchvehicle').store.pageSize;
	...
	onExcelClick :　function() {
		...
		if(Ext.getCmp('dispatchvehicle_SearchForm_id').getForm().isValid()){	
			...
			window.location.href = './resources/template/reportmgmt/usage_report.xlsx';
		}else{
        	Ext.Msg.alert('消息提示','请输入有效的筛选条件！');
        }
	},
	...
	openDeptChooseWin: function(combo, event, eOpts){
     	var win = Ext.create("Admin.view.reportmgmt.dispatchvehicle.DeptChooseWin",{
  	...
  	chooseDept: function(btn, e, eOpts){
     	...
     	var form = Ext.getCmp("dispatchvehicle_SearchForm_id").getForm();
