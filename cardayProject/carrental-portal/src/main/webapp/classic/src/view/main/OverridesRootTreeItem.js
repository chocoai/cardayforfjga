Ext.define('Admin.view.main.OverridesRootTreeItem', {
		...
		if (me.treeItemFloated !== floated) {
			...
			// toolkit-specific
				...
				if (!wasExpanded && node.isExpanded()) {
					// If we have been floating and expanded a child, we may
					// have been
					// expanded as part of the ancestors. Attempt to restore
					// state.
				...
				placeholder.dom.parentNode
						.insertBefore(el.dom, placeholder.dom);
				placeholder.destroy();
				me.floater.destroy();
	...
	privates : {
		createFloater : function() {
			var me = this, owner = me.getOwner(), ownerTree = me.up('treelist'), floater, toolElement = me
					.getToolElement();
			me.floater = floater = new Ext.container.Container({
						...
						width : Ext.isIE8
								? 200
								: (ownerTree.expandedWidth - toolElement
										.getWidth()),
						shadow : false,
