 ...
 */
Ext.define('Admin.view.reportmgmt.coststatistics.ViewController', {
	extend : 'Ext.app.ViewController',
	requires : [ 
			'Ext.window.MessageBox'
	],
	alias : 'controller.coststatisticscontroller',
	...
	onBeforeLoad : function() {		
 		...
 		this.getViewModel().getStore("vehicleList").load();
			...
			if(Ext.getCmp('coststatistics_SearchForm_id').getForm().isValid()){
				var page=Ext.getCmp('GridVehicle_page_coststatistics').store.currentPage;
				var limit=Ext.getCmp('GridVehicle_page_coststatistics').store.pageSize;
	...
	onExcelClick :　function() {
		...
		if(Ext.getCmp('coststatistics_SearchForm_id').getForm().isValid()){	
			...
			window.location.href = './resources/template/reportmgmt/costs_report.xlsx';
		}else{
        	Ext.Msg.alert('消息提示','请输入有效的筛选条件！');
        }
	},
	...
	openDeptChooseWin: function(combo, event, eOpts){
     	var win = Ext.create("Admin.view.reportmgmt.coststatistics.DeptChooseWin",{
  	...
  	chooseDept: function(btn, e, eOpts){
     	...
     	var form = Ext.getCmp("coststatistics_SearchForm_id").getForm();
