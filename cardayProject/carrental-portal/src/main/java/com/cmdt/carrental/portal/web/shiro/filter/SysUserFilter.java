...
public class SysUserFilter extends PathMatchingFilter {
    ...
    protected boolean onPreHandle(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception {
        ...
        try{
        	...
        	// 判断用户是否拥有特殊警务的权限
            if (SecurityUtils.getSubject().isPermitted(com.cmdt.carrental.common.constants.Constants.SPECIAL_SERVICE)) {
                user.setHasSpecialServicePerm(true);
            } else {
                user.setHasSpecialServicePerm(false);
            }
            
//            if (user.getUserCategory() == 12  || user.getUserCategory() == 13 || user.getUserCategory() == 14) {
//            	user.setUserCategory(6L);
//            }
