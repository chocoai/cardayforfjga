...
import com.cmdt.carrental.platform.service.model.request.alert.AlertVehicleQueryDto;
...
public class PlatformVehicleAlertService {
	...
	public PagModel findAlertByType(User loginUser, VehicleAlertListDto dto) throws Exception {
		QueryAlertInfoModel model = new QueryAlertInfoModel();
		BeanUtils.copyProperties(model, dto);
		return vehicleAlertService.findVehicleAlertInfo(model);
	...
	public VehicleOutBoundModel findMarkerByVehicleNumber(User loginUser, AlertMarkerVehicleDto dto) throws Exception {
		...
		if (markerIdlist != null && !markerIdlist.isEmpty()) {
			marker = markerIdlist.get(0);// 拿到carday的markerid根据车牌号
		}
		// 根据车牌号查询设备号
		...
		vehicleOutBoundModel.setMarker(marker);
