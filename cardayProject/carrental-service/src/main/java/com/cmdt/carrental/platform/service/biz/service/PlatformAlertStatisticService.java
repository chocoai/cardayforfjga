...
public class PlatformAlertStatisticService {
	...
	public AlertOrgStatisticRetDto statisticAlertByTypeAndTimeRanger(AlertOrgStaticDto dto){
		...
		if(orgList!=null&&orgList.size()>0){
			 List<AlertStatisticModel> data = vehicleAlertJobDao.statisticAlertByTypeAndTimeRanger(orgList,dto.getEntId(),false,true,dto.getType(),dto.getFrom(),dto.getTo());
			 return new AlertOrgStatisticRetDto(dto.getType(),dto.getFrom(),dto.getTo(),data);
		}else{
			return new AlertOrgStatisticRetDto(dto.getType(),dto.getFrom(),dto.getTo(),new ArrayList<AlertStatisticModel>());
		}
	}
	...
	public AlertDailyStatisticRetDto statisticDailyAlertByTypeAndTimeRanger(AlertOrgDailyStatisticDto dto){
		 ...
		 List<Organization> orgList = organizationService.findDownOrganizationListByOrgId(Long.parseLong(dto.getEntId()), false, true);
		 if(orgList!=null&&orgList.size()>0){
			 ...
			 List<CountModel> result = vehicleAlertJobDao.statisticDailyAlertByTypeAndTimeRanger(orgIdList,Long.parseLong(dto.getEntId()), false,dto.getType(),dto.getFrom(),dto.getTo());
	...
	public void generateAlertByType(AlertCreateDto alertCreateDto){
		...
		if(null!=vehicleModel){
			...
			if(StringUtils.isNotEmpty(alert.getAlertSpeed())){
				double os = (double)(alertCreateDto.getSpeed()-vehicleModel.getLimitSpeed())/vehicleModel.getLimitSpeed();
